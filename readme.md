# **AgentKon**: A Google Gemini Enterprise, ADK & MCP Hackathon 🚀

Welcome to **AgentKon**, our hands-on hackathon dedicated to exploring and building with Google's powerful agent development tools: **Gemini Enterprise**, the **Agent Development Kit (ADK)**, and **Multi-agent Control Plane (MCP)**.

This repository contains the agenda and resources for our session. Throughout the day, you'll participate in four guided labs and a final group brainstorming exercise to apply your new skills. Here is a sample agenda for the day.

> [!NOTE]
> Your Google Cloud team is also eager to tailor this event to meet your needs more specifically. Please let us know if you have any requests or suggestions by reaching out to your Google Cloud CE or account manager.

---

## 📋 Sample Agenda

| Time | Topic | Presenter / Notes |
| :--- | :--- | :--- |
| 09:00 - 09:30 | 🥐 **Arrival & Breakfast** | • Arrival at our Munich office: Erika-Mann-Str. 33, 80635 Munich.<br>• Room: **"10x"**. Breakfast will be served. |
| 09:30 - 09:45 | 👋 **Welcome & Introduction** | • Welcome to **AgentKon** & Opening Remarks<br>• Round of Attendee Introductions (All) |
| 09:45 - 10:15 | 💻 **Gemini Enterprise Demo** | • A short presentation and demo of the Gemini Enterprise frontend. |
| 10:15 - 11:00 | 🛠️ [**Lab 1: Environment Setup**](./labs/lab1-environment-setup/README.md) | • Access your temporary Devstar Environment.<br>• Google Cloud Console overview.<br>• Create your own no-code/low-code agent. Be creative! |
| 11:00 - 12:00 | 🏗️ [**Lab 2: Building with ADK**](./labs/lab2-building-with-adk/README.md) | • Learn how to build your first custom full-code ADK agent. |
| 12:00 - 13:00 | 🍴 **Lunch Break** | Enjoy some delicious food! |
| 13:00 - 13:10 | ⚡ **Energizing Activity** | • A short and fun activity to get our energy levels back up! |
| 13:10 - 14:15 | 🧰 [**Lab 3: ADK + MCP Toolbox**](./labs/lab3-adk-mcp-toolbox/README.md) | • Build a Travel Agent using MCP Toolbox for Databases and ADK. |
| 14:15 - 14:30 | ☕ **Coffee Break** | Recharge with some caffeine. |
| 14:30 - 15:55 | 🚀 [**Lab 4: Advanced Agents**](./labs/lab4-advanced-agents/README.md) | • [Agent Design Patterns Deck](https://github.com/WGabriel/agentspace-hackathon/blob/697fce8bbe6955d02c7865940c5d0c16a370136c/labs/lab4/assets/agent-design-patterns.pdf)<br>• Running your first Multi-Agent Travel Concierge Bot<br>• Lab 4.1: Google Search integration<br>• **Lab 4.2: External MCP server for real-time results**<br>• Deploying to Gemini Enterprise |
| 15:55 - 16:00 | 📝 **Feedback Form** | • Your feedback is important! [Fill it out here](https://forms.gle/nAT4pWsbM34pAS3U7). |
| 16:00 - 16:30 | 🧠 **Group Exercise** | • Recap of the labs.<br>• **Brainstorming:** How can you apply what you learned today? |
| **💫 Bonus** | 🎆 **A2A Multi-Agent Systems** | • [Bonus Lab: InstaVibe ADK Multi-Agents](https://codelabs.developers.google.com/instavibe-adk-multi-agents/instructions#0)<br>• Learn the Agent-to-Agent (A2A) protocol for distributed agents. |

---

## 🧠 Group Exercise: Brainstorming

To conclude the day, you will form groups and brainstorm a practical application for the technologies covered.

### 🎯 Your Goal:

1.  **Identify a use case** within your company where agent technology could be applied.
2.  **Analyze the impact:** How could this solution **reduce expenses** or **increase revenue**?
3.  **Prepare a pitch:** Create a [one-slider](https://docs.google.com/presentation) outlining your idea.
4.  **Present:** Each group will select one person to present the idea.

Happy building! 🚀
